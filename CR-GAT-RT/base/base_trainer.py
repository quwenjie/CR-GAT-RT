...
class BaseTrainer:
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, train_logger=None):
        ...
        self.start_epoch = 1
        self.improved = False
        self.model.set_mean_std(train_loader.MEAN,train_loader.STD)
        # SETTING THE DEVICE
        ...
        #                            'lr': config['optimizer']['args']['lr'] / 10}]
        #else:
        self.optimizer = get_instance(torch.optim, 'optimizer', config, filter(lambda p:p.requires_grad, self.model.parameters()))
